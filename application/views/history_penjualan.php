		...
		<div class="normal-table-list">
			<table class="table table-sc-ex">
				<thead style='background-color:#FFFFFFF'>
					<form method="POST" action="<?php echo base_url().'index.php/laporan/search_history_penjualan/'; ?>">
					<tr>
						<th>
						...
						<tbody>
							...
							<tr>
								...
								<?php 
									...
									if($d['serial'] == 1){
										echo count($sub_transaksi);
									} else {
										foreach($sub_transaksi as $st){
											$unit = $st['unit'];
											$total_unit[] = $unit;
										}
										echo '<a href="'.base_url().'index.php/laporan/history_penjualan_item/'.$d['id'].'^'.$month.'^'.$year.'^'.$cabang.'^'.$seles.'">'.array_sum($total_unit).'</a>';
									}	
								?>
								</td>
								<td>
