...
<div class="content table-responsive table-full-width">
	<table class="table table-striped m-b-none" style="width:100%">
		<thead>
			<form method="POST" action="<?php echo base_url().'index.php/welcome/search_stok_opname' ?>">
			<th>Cabang : 
			<?php if($check_admin) { ?>	
			<select name="cabang">
	...
	<table class="table table-striped m-b-none" style="width:100%">	
		...
		<tbody>
				...
				<tr>
					...
					<td>
					<?php
					$first_date = strtotime('2019-05-25');
					if(!empty($tanggal)){
						$this->db->where('waktu >=', date('Y-m-d', $first_date));
						$this->db->where('waktu <=', date('Y-m-d',	$tanggal));
					}
					$this->db->where('id_cabang', $cabang);	
					...
					if(!empty($tanggal)){
						$this->db->where('tgl_masuk >=', date('Y-m-d', $first_date));
						$this->db->where('tgl_masuk <=', date('Y-m-d',$tanggal));
					}
				...
				<form>
					...
					<td>
						<?php
							...
							if(empty($stok_item_keluar)){
								$stok_item_keluar = 0;
							}
							
							if(empty($stok_total_penjualan)){
								$stok_total_penjualan = 0;
							}
							...
							$total = (($stok_awal_masuk + $stok_item + $mutasi_masuk) - ($stok_item_keluar + $stok_total_penjualan + $mutasi_keluar));
							echo $total;
						?>
					</td>
					<td>
					<?php 
							...
							$this->db->where('id_item', $d['id']);
							$this->db->where('id_cabang', $cabang);
							$stok_laporan = $this->db->get('stok_laporan')->result_array();
							
							if(!empty($stok_laporan)){
								$stok_laporan =  $stok_laporan[0]['stok'];
							} else {
								$stok_laporan =  0;
							}	
							echo $stok_laporan;
					...
					<td>
						<?php 
						$total_result = $stok_laporan - (($stok_awal_masuk + $stok_item + $mutasi_masuk) - ($stok_item_keluar + $stok_total_penjualan + $mutasi_keluar));
						echo $total_result;
