				...
				<div class="bsc-tbl">
					<table class="table table-sc-ex">
						<thead style='background-color:#FFFFFFF'>
							<form method="POST" action="<?php echo base_url().'index.php/stok/search_return_stok/'; ?>">
							<tr>
					...
					<table class="table table-sc-ex">
						...
						<tbody>
							...
							<tr>
								...
								<td><button type="button" class="btn btn-<?php echo $warna; ?>" data-toggle="modal" data-target="#proses_return<?php echo $d['id']; ?>"><?php echo $proses[$d['status']]; ?></button></td>
								<td>
									<button type="submit" class="btn btn-info" data-toggle="modal" data-target="#ubah_return<?php echo $d['id']; ?>">Ubah</button>
									<a href='<?php echo base_url().'index.php/stok/daftar_return_stok/'.$d['id'].'^'; ?>'><span class="btn btn-success"> Detail</span></a>
									<?php if($d['status'] == 0 || $check_admin){ ?>
									<a href='<?php echo base_url().'index.php/persedian_page/delete_item/return_item/'.$d['id']; ?>' onclick="return confirm('Are you sure delete this item?');">
	...
	<div class="text-center">
		<?php
			...
			if($posisi > 0){
				...
				$href = base_url().'index.php/stok/'.$this->uri->segment(2).'/'.$link.'/'.$page;
			} else {
				$page = $this->uri->segment(3) - 10;
				$href = base_url().'index.php/stok/'.$this->uri->segment(2).'/'.$page;
			}
...
<div class="modal fade" id="return_stok" role="dialog">
	<div class="modal-dialog modal-large">
		<div class="modal-content">
			...
			<form method="POST" action="<?php echo base_url().'index.php/stok/add_return_stok/'; ?>">
