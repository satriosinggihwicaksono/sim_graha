						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<?php if(empty($data)){ ?><a href='<?php echo base_url().'index.php/stok/remove_trans_item/'.$id_trans; ?>' onclick="return confirm('Are you sure delete this item?');"><span class="btn btn-danger fa fa-trash-o"> Hapus</span></a> <?php } ?>
								</div>
							</div>
						</div>
				<?php if(($id_from == $id_cabang && $status == 0) || $check_admin){ ?>	
				<form method="POST" action="<?php echo base_url().'index.php/stok/search_daftar_item/'; ?>">
						...
						<tbody>
							...
							<tr>
								...
								<td style="text-align:center; width:100px;">
									<?php if($status != 1){?>
									<a href='<?php echo base_url().'index.php/stok/add_daftar_item/'.$id_daftar.'/'.$item[0]['id'].'/'.$s['id']; ?>'><span class="btn btn-success"> Tambah</span></a>
									<?php } else { ?>
									<a href='<?php echo base_url().'index.php/stok/add_daftar_item_status/'.$id_daftar.'/'.$item[0]['id'].'/'.$s['id'].'/'.$id_to; ?>'><span class="btn btn-success"> Tambah</span></a>									
									<?php }?>
								</td>
							</tr>
							<?php 
							...
							?>
								...
								<td style="text-align:center; width:100px;">
									<?php if($status != 1){?>
									<a href="<?php echo base_url().'index.php/stok/add_daftar_item/'.$id_daftar.'/'.$i["id"].'/0/0'?>"> <span class="btn btn-success"> Tambah</span></a>
									<?php } else { ?>
									<a href="<?php echo base_url().'index.php/stok/add_daftar_item_status/'.$id_daftar.'/'.$i["id"].'/0/0/'.$id_to?>">	<span class="btn btn-success"> Tambah</span></a>
									<?php } ?>
								</td>
							</tr>
							<?php	
						...
						<tbody>
							...
							<tr>
								...
								<td style="text-align:center; width:50px;">
									...
									<form method="POST" action="<?php echo base_url().'index.php/stok/ubah_unit/'.$d['id']; ?>">
								...
								<?php if(($id_from == $id_cabang && $status == 0) || $check_admin ){ ?>
								<td style="text-align:center; width:100px;">	
									<a href='<?php echo base_url().'index.php/persedian_page/delete_barang/trans_item/'.$d['id']; ?>' onclick="return confirm('Are you sure delete this item?');"><span class="btn btn-danger fa fa-trash-o"> Hapus</span></a>
								</td>
								<?php } ?>
							</tr>
							<?php } ?>
				...
				<div class="rc-it-ltd">
					<div class="recent-items-ctn">
						<div class="text-align:center;">
							<h2><a href="<?php echo base_url().'index.php/printer/printTransfer/'.$id_daftar;?>" onclick="window.open('<?php echo base_url().'index.php/printer/printTransfer/'.$id_daftar;?>', 'newwindow', 'width=600, height=700'); return false;"><button onclick="demo.showNotification('top','left')" class="fa fa-print"> CETAK</button></a></h2>
						</div>
					</div>
				</div>
			</div>
