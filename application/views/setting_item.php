				...
				<div class="row">
					...
					<div class="col-md-2">
						<div class="form-group">
							...
							<form method="POST" action="<?php echo base_url().'index.php/stok/proses_laporan_stok/'; ?>">
			...
			<div class="bsc-tbl">
					<table class="table table-sc-ex" style="width:50%">
						<thead style='background-color:#FFFFFFF'>
							<form method="POST" action="<?php echo base_url().'index.php/persedian_page/search_setting_item/'; ?>">
							<tr>
				...
				<table class="table table-sc-ex" width="100%" cellspacing="0">
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="<?php echo base_url().'index.php/stok/history_pembelian_stok/'.$d['id'].'^^^^^'; ?>"><?php echo $d['nama']; ?></a></td>
							...
							<?php 
								...
								foreach($cabang as $c){
									if($d['serial'] == 1){
										...
										$url = base_url().'index.php/stok/tambah_stok/'.$d["id"].'^'.$c["id"].'^^';
										echo "<td><a href='".$url."'</a>".$total_stok."</td>";
									} else {
										// pembelian
										$pembelian_unit = array();
										$this->db->select('sub_pembelian.unit');
										...
										$stok_keluar_unit = array_sum($stok_keluar_unit) * -1;
										...
										$total = $pembelian_unit + ($to_unit - ($from_unit + $transaksi_unit)) - $return_unit + $stok_keluar_unit;
							...
							<td>
								...
								if($d['serial'] == 1){
								?>
								<a href='<?php echo base_url().'index.php/stok/tambah_stok/'.$d['id'].'^0^^'; ?>'><?php echo array_sum($total_keseluruhan); ?></a>
								<?php } else {
							...
							<td style="text-align:center">
								...
								<a href='<?php echo base_url().'index.php/persedian_page/delete_barang/item/'.$d['id']; ?>' onclick="return confirm('Menghapus daftar item dapat menghilangkan semua history?');">
	...
	<div class="text-center">
		<?php
			...
			if($posisi > 0){
				...
				$href = base_url().'index.php/persedian_page/'.$this->uri->segment(2).'/'.$link.'/'.$page;
			} else {
